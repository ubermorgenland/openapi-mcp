...
# YouTube Transcript SSE Simple Test
# Tests the /youtube-transcript/sse endpoint (Server-Sent Events) with authentication
...
SSE_ENDPOINT="/youtube-transcript/sse"
MESSAGE_ENDPOINT="/youtube-transcript/message"
...
print_header "YouTube Transcript SSE Simple Test"
...
if grep -q "transcript" "$sse_log" || echo "$list_response" | grep -q "transcript"; then
...
print_test "Call transcript tool with explicit RapidAPI key via SSE"
...
print_test "Call transcript tool with header authentication via SSE"
...
echo ""
echo "📝 NOTE: This test uses different video IDs and 3-second delays between requests"
echo "    to avoid rate limiting while thoroughly testing all authentication methods."
echo ""
print_header "SSE Test Complete"
echo "✅ YouTube Transcript SSE authentication test completed successfully"